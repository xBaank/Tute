    ...
    public override void OnInspectorGUI()
    {
        ...
        using (new HorizontalScope(GUI.skin.box))
        {
            effect.whiteNoiseFrequency = EditorGUILayout.IntField(
                "White Noise Freaquency (x/1000)",
                effect.whiteNoiseFrequency
            );
            effect.whiteNoiseLength = EditorGUILayout.FloatField(
                "White Noise Time Left (sec)",
        ...
        using (new VerticalScope(GUI.skin.box))
        {
            effect.screenJumpFrequency = EditorGUILayout.IntField(
                "Screen Jump Freaquency (x/1000)",
                effect.screenJumpFrequency
            );
            ...
            {
                effect.screenJumpMinLevel = EditorGUILayout.FloatField(
                    "min",
                    effect.screenJumpMinLevel
                );
                effect.screenJumpMaxLevel = EditorGUILayout.FloatField(
                    "max",
        ...
        using (new HorizontalScope(GUI.skin.box))
        {
            effect.isMonochrome = EditorGUILayout.Toggle(
                "Monochrome On / Off",
                effect.isMonochrome
            );
        }
        ...
        using (new VerticalScope(GUI.skin.box))
        {
            ...
            effect.isSlippageNoise = EditorGUILayout.Toggle(
                "Slippage Noise",
                effect.isSlippageNoise
            );
            effect.slippageStrength = EditorGUILayout.FloatField(
                "Slippage Strength",
                effect.slippageStrength
            );
            effect.slippageInterval = EditorGUILayout.FloatField(
                "Slippage Interval",
                effect.slippageInterval
            );
            effect.slippageScrollSpeed = EditorGUILayout.FloatField(
                "Slippage Scroll Speed",
        ...
        using (new VerticalScope(GUI.skin.box))
        {
            effect.isMultipleGhost = EditorGUILayout.Toggle(
                "Multiple Ghost On / Off",
                effect.isMultipleGhost
            );
            effect.multipleGhostStrength = EditorGUILayout.FloatField(
                "Multiple Ghost Strength",
        ...
        using (new VerticalScope(GUI.skin.box))
        {
            ...
            effect.letterBoxType = (LeterBoxType)
                EditorGUILayout.EnumPopup("Letter Box Type", effect.letterBoxType);
        }

        using (new VerticalScope(GUI.skin.box))
        {
            effect.isDecalTex = EditorGUILayout.Toggle("Decal Tex On / Off", effect.isDecalTex);
            effect.decalTex = (Texture2D)
                EditorGUILayout.ObjectField("Decal Tex", effect.decalTex, typeof(Texture2D), false);
            effect.decalTexPos = EditorGUILayout.Vector2Field(
                "Decal Tex Position",
                effect.decalTexPos
            );
            effect.decalTexScale = EditorGUILayout.Vector2Field(
                "Decal Tex Scale",
        ...
        using (new VerticalScope(GUI.skin.box))
        {
            effect.isLowResolution = EditorGUILayout.Toggle(
                "Low Resolution",
